...
class Generator(nn.Module):
    ...
    def __init__(self, emb_dim, proj_emb_dim=128, noise_dim=100, num_gen_features=64):
        ...
        self.latent_dim = (
            noise_dim + proj_emb_dim
        )  # The size of the latent space for the generator
        ...
        self.proj_emb_net = nn.Sequential(
            nn.Linear(in_features=self.emb_dim, out_features=self.proj_emb_dim),
...
class GAN_INT_CLS(nn.Module):
    ...
    def generate_images(self, captions):
        ...
        with torch.no_grad():
            self.generator.eval()
            emb_cap = (
                self.emb_net.encode_text(captions.to(self.device))
                .to(self.device)
                .float()
            )
            z = (
                torch.distributions.Normal(0.0, 0.1)
                .sample((captions.size(0), self.noise_dim))
                .to(self.device)
            )
    ...
    def fit(
        ...
        logger = logger or CSVLogger(
            os.path.join(os.getcwd(), "logs"), name=self._get_name()
        )
        ...
        logger.log_hyperparams(
            {
                "epochs": num_epochs,
                "gen optimizer": type(gen_optim),
                "gen optimizer_kwargs": gen_optim.defaults,
                "disc optimizer": type(disc_optim),
                "disc optimizer_kwargs": disc_optim.defaults,
        ...
        def sample_wrong_embeddings(dataloader, classes):
            ...
            for i in range(emb_cap.size(0)):
                _, w_cap, w_class = dataloader.dataset[
                    random.randint(0, len(dataloader.dataset) - 1)
                ]
        ...
        def process_batch(dataloader, images, emb_cap, classes):
            ...
            z = (
                torch.distributions.Normal(0.0, 0.1)
                .sample((images.size(0), self.noise_dim))
                .to(device)
            )
            fake_images = self.generator(emb_cap, z)
        ...
        for epoch in range(num_epochs):
            ...
            for total_processed_batches, (images, captions, classes) in enumerate(
                train_dataloader
            ):
                ...
                with torch.no_grad():
                    emb_cap = (
                        self.emb_net.encode_text(captions).to(device).float()
                    )  # Encode captions embeddings

                gen_optim.zero_grad()
                disc_optim.zero_grad()

                real_loss, fake_loss, wrong_loss, real_acc, fake_acc, wrong_acc = (
                    process_batch(train_dataloader, images, emb_cap, classes)
                )
            ...
            metrics = {
                ...
                "train_real_acc": running_real_acc,
                "train_fake_acc": running_fake_acc,
                "train_wrong_acc": running_wrong_acc,
            }
            ...
            if val_dataloader is not None:
                ...
                with torch.no_grad():
                    total_val_loss = 0.0
                    (
                        val_running_real_acc,
                        val_running_fake_acc,
                        val_running_wrong_acc,
                    ) = (0.0, 0.0, 0.0)
                    ...
                    for idx, (images, captions, classes) in enumerate(val_dataloader):
                        ...
                        (
                            ...
                            fake_acc,
                            wrong_acc,
                        ) = process_batch(val_dataloader, images, emb_cap, classes)
                        ...
                        v_bar.update(1)
                        v_bar.set_description(
                            f"Validation => Val Loss: {total_val_loss / (idx+1):.6f}"
            ...
            if save_path is not None:
                ...
                torch.save(
                    save_obj,
                    os.path.join(save_path, f"{self._get_name()}_epoch-{epoch+1}.pt"),
                )
    ...
    def summary(self, batch_size=1):
        ...
        # Generator summary
        summary(
            self.generator, [(batch_size, self.emb_dim), (batch_size, self.noise_dim)]
        )

        # Discriminator summary
        summary(
            self.discriminator, [(batch_size, 3, 64, 64), (batch_size, self.emb_dim)]
        )
